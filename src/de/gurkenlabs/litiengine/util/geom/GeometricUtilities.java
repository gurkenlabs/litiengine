...
public class GeometricUtilities {
  ...
  public static boolean shapeIntersects(final Shape shapeA, final Shape shapeB) {
    // compute rough estimate of boundary intersection to avoid more costly checks if not
    // intersecting
    ...
    if (shapeA instanceof Line2D) {
      if (shapeB instanceof Line2D) {
        return ((Line2D) shapeA)
            .intersectsLine((Line2D) shapeB); // intersectsLine() not defined on Shape thus casting
      }
