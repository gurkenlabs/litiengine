...
public class ListField extends GuiComponent {
  ...
  public void render(final Graphics2D g) {
    ...
    if (this.selectedComponent != null) {
      ...
      border = new Rectangle2D.Double(selectedComponent.getX() - 1, selectedComponent.getY() - 1, borderWidth, borderHeight);
  ...
  public void setSelection(final int column, final int row) {
    if (column < 0 || column >= nbOfColumns || row < 0 || row >= getContent()[column].length) {
      return;
    }
    ...
    } else if (selectionRow < getVerticalLowerBound() && getVerticalLowerBound() > 0) {
      setVerticalLowerBound(getVerticalLowerBound() - 1);
    }
    if (selectionColumn >= getHorizontalLowerBound() + getNumberOfShownColumns()) {
      setHorizontalLowerBound(getHorizontalLowerBound() + 1);
    } else if (selectionColumn < getHorizontalLowerBound() && getHorizontalLowerBound() > 0) {
      setHorizontalLowerBound(getHorizontalLowerBound() - 1);
    }
  ...
  private void initContentList() {
    ...
    for (int column = 0; column < getNumberOfShownColumns(); column++) {
      ...
      for (int row = 0; row < getNumberOfShownRows(); row++) {
        ...
        if (getContent()[column][row] == null) {
          entryComponent = new ImageComponent(getX() + (columnWidth * column), getY() + (rowHeight * row), columnWidth, rowHeight, entrySprite, "", null);
        } else if (getContent()[column][row] instanceof Image image) {
          entryComponent = new ImageComponent(getX() + (columnWidth * column), getY() + (rowHeight * row), columnWidth, rowHeight, entrySprite, "", image);
        } else {
          entryComponent = new ImageComponent(getX() + (columnWidth * column), getY() + (rowHeight * row), columnWidth, rowHeight, entrySprite, getContent()[column][row].toString(), null);
        }
        if (isSliderInside() && getVerticalSlider() != null) {
          ...
          entryComponent.setWidth(entryComponent.getWidth() - (getVerticalSlider().getWidth() / getNumberOfShownColumns()));
        }
        if (isSliderInside() && getHorizontalSlider() != null) {
          entryComponent.setY(getY() + ((rowHeight - (getHorizontalSlider().getHeight() / getNumberOfShownRows())) * row));
    ...
    this.onChange(s -> {
      if (getVerticalSlider() != null) {
        ...
        getVerticalSlider().getSliderComponent().setLocation(getVerticalSlider().getRelativeSliderLocation());
      }
      if (getHorizontalSlider() != null) {
        getHorizontalSlider().setCurrentValue(getHorizontalLowerBound());
        getHorizontalSlider().getSliderComponent().setLocation(getHorizontalSlider().getRelativeSliderLocation());
      }
    });
    if (getVerticalSlider() != null) {
      getVerticalSlider().onChange(sliderValue -> {
    ...
    if (getHorizontalSlider() != null) {
      getHorizontalSlider().onChange(sliderValue -> {
  ...
  private void initSliders() {
    ...
    if (this.getNumberOfShownColumns() < getContent().length) {
      if (this.isSliderInside()) {
        this.horizontalSlider = new HorizontalSlider(getX(), getY() + getHeight() - sliderSize, getWidth() - sliderSize, sliderSize, 0, (float) nbOfColumns - getNumberOfShownColumns(), 1);
      } else {
        this.horizontalSlider = new HorizontalSlider(getX(), getY() + getHeight(), getWidth(), sliderSize, 0, (float) nbOfColumns - getNumberOfShownColumns(), 1);
      }
    ...
    if (maxNbOfRows > 0) {
      if (isSliderInside()) {
        if (getHorizontalSlider() != null) {
          this.verticalSlider = new VerticalSlider(getX() + getWidth() - sliderSize, getY(), sliderSize, getHeight() - sliderSize, 0, (float) getMaxRows() - getNumberOfShownRows(), 1);
        } else {
          this.verticalSlider = new VerticalSlider(this.getX() + this.getWidth() - sliderSize, this.getY(), sliderSize, this.getHeight(), 0, (float) this.getMaxRows() - this.getNumberOfShownRows(), 1);
        }
      } else {
        this.verticalSlider = new VerticalSlider(this.getX() + this.getWidth(), this.getY(), sliderSize, this.getHeight(), 0, (float) this.getMaxRows() - this.getNumberOfShownRows(), 1);
      }
  ...
  private void refreshListEntries() {
    for (int column = 0; column < getNumberOfShownColumns(); column++) {
      for (int row = 0; row < getNumberOfShownRows(); row++) {
        ...
        if (row + getVerticalLowerBound() < getContent()[column + getHorizontalLowerBound()].length && getContent()[column + getHorizontalLowerBound()][row + getVerticalLowerBound()] != null) {
          if (getContent()[column + getHorizontalLowerBound()][row + getVerticalLowerBound()] instanceof Image image) {
            ...
            getListEntry(column, row).setText(getContent()[column + getHorizontalLowerBound()][row + getVerticalLowerBound()].toString());
          }
        } else {
  ...
  private void selectComponent() {
    if (!isEntireRowSelected() && getSelectionColumn() >= getHorizontalLowerBound() && getSelectionColumn() < getHorizontalLowerBound() + getNumberOfShownColumns() && getSelectionRow() >= getVerticalLowerBound() && getSelectionRow() < getVerticalLowerBound() + getNumberOfShownRows()) {
      this.selectedComponent = getListEntry(getSelectionColumn() - getHorizontalLowerBound(), getSelectionRow() - getVerticalLowerBound());
    } else if (isEntireRowSelected() && getSelectionColumn() >= 0 && getSelectionColumn() < nbOfColumns && getSelectionRow() >= getVerticalLowerBound() && getSelectionRow() < getVerticalLowerBound() + getNumberOfShownRows()) {
