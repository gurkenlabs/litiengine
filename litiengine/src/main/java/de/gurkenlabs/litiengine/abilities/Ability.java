...
public abstract class Ability implements IRenderable {
  ...
  public Ellipse2D calculatePotentialImpactArea() {
    final int range = getAttributes().impact().getCurrent();
  ...
  public float getCooldownInSeconds() {
    return (float) (getAttributes().cooldown().getCurrent() * 0.001);
  }
  ...
  public float getRemainingCooldownInSeconds() {
    ...
    return (float) (!canCast()
      ? (getAttributes().cooldown().getCurrent()
  ...
  protected Shape internalCalculateImpactArea(final double angle) {
    final int impact = getAttributes().impact().getCurrent();
    final int impactAngle = getAttributes().impactAngle().getCurrent();
    ...
    final Point2D appliedRange =
      GeometricUtilities.project(
        new Point2D.Double(arcX, arcY), angle, getAttributes().range().getCurrent() * 0.5);
