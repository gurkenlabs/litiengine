...
public final class TextRenderer {
  ...
  public static void render(final Graphics2D g, final String text, Rectangle2D bounds, Align align, Valign valign, double offsetX, double offsetY,
    ...
    double locationX = bounds.getX() + align.getLocation(bounds.getWidth(), g.getFontMetrics().stringWidth(text), true) + offsetX;
    double locationY = bounds.getY() + valign.getLocation(bounds.getHeight(), getHeight(g, text), true) + g.getFontMetrics().getAscent() + offsetY;
  ...
  public static void renderWithOutline(final Graphics2D g, final String text, final double x, final double y, final Color outlineColor,
    final boolean antiAliasing) {
    float stroke = (float) Math.clamp((g.getFont().getSize2D() * 1 / 5f) * Math.log(Game.world().camera().getRenderScale()), 1, 100);
