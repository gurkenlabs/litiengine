...
public class UpdatedEmitterPanel extends PropertyPanel {
  ...
  public UpdatedEmitterPanel() {
    ...
    for (EmitterPropertyGroup e : EmitterPropertyGroup.values()) {
      ...
      this.propertyGrouptabs.insertTab(String.format("<html><p style=\"text-align: left; width: %spx\">%s</p></html>", LABEL_WIDTH * 1.5, localized), null, UpdatedEmitterPropertyPanel.getEmitterPropertyPanel(e), Resources.strings().get(String.format("emitter_tip_%s", e.name().toLowerCase())),
          e.ordinal());
    }
  ...
  public void bind(IMapObject mapObject) {
    for (EmitterPropertyGroup e : EmitterPropertyGroup.values()) {
      ((UpdatedEmitterPropertyPanel) this.propertyGrouptabs.getComponent(e.ordinal())).bind(mapObject);
    }
  }
